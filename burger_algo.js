...
async function generateIngredients(IdealProductionValues, token) {
  ...
  async function sendBurger(burgerSKU, token, kitchenId, quantity) {
    // Iterate over the MinimumValues dictionary
        let productidlistobject = await axios.get(`https://dev.api-proyecto.2023-1.tallerdeintegracion.cl/warehouse/stores/${kitchenId}/products?sku=${burgerSKU}`, {
            headers: {
            Authorization: `Bearer ${token}`, // ADD TOKEN !!!!!!!!!!
            },
            });
        ...
        for (let i = 0; i < quantity; i++) {
                ...
                try {
                    // Execute the PATCH call for the current productId
                    await axios.post(
                        `https://dev.api-proyecto.2023-1.tallerdeintegracion.cl/warehouse/products/${productids[i]}/group`, {
                      group: 1
                    }, {
                      headers: {
                        Authorization: `Bearer ${token}`, // ADD TOKEN!!!!!
                    ...
                    console.log(`Moved burger ${productids[i]}`);
                  } catch (error) {
                    console.error(`Error moving burger ${productids[i]}: ${error.message}`);
                  }
              }
  ...
  async function burger() {
    ...
    async function executeLoop() {
      ...
      console.log(total);
      if (total <= 9) {
        // Continue the loop after 2 minutes
        setTimeout(executeLoop, 2 * 60 * 1000);
      } else {
        console.log("Total is greater than 9, stopping the loop!");
        //wait 3 minutes
        setTimeout(async () => {
            let resultfinal = await sendBurger(burgerId, result1.token, result1.kitchenId, 10);
          }, 240000);
      }
    }
